...
output "ssh_command" {
  ...
  value       = "cloudflared access ssh --hostname ssh-samuel-tenga.nona.company"
}
...
output "ssh_config" {
  ...
  value       = <<-EOT
    Host nexus
      HostName ssh-samuel-tenga.nona.company
      User root
...
output "service_urls" {
  ...
  value = {
    ...
    key => "https://${service.subdomain}-samuel-tenga.nona.company"
  }
}
...
output "secrets" {
  ...
  value = {
    ...
    redpanda_schema_registry_public_url = "http://redpanda-schema-registry-samuel-tenga.nona.company:18081"
